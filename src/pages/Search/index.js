...
export default function Search({ setSideMenuVisibility }) {
  ...
  refinements &&
    Object.keys(refinements).map(key => {
      refinements[key].map(item => checkboxValues = { ...checkboxValues, [item.value]: false })
    });
  ...
  function handleCheckbox(e) {
    const { target } = e;
    setCheckboxes(prevState => ({ ...prevState, [target.name]: !checkboxes[target.name] }));
  }
  /***/
  return (
    <>
        ...
        <S.Left >
          {refinements ?
            <>
                  ...
                  {refinements[key].map((item) =>
                    <S.FilterItem key={item.value}>
                      <S.Label >
                        {key === 'reviews' ?
                          <>
                            <S.CheckBox align='flex-end'  checked={checkboxes[item.value] || false} onChange={handleCheckbox} name={item.value} />
                            <S.Stars stars={item.name.substr(0, 1)} />
                            & Up
                          </>
                          :
                          <>
                            <S.CheckBox checked={checkboxes[item.value] || false} onChange={handleCheckbox} name={item.value} />
                            {item.name}
                          </>
                        }
                      </S.Label>
              ...
              < Button width='92%' onClick={handleFiltersBtn}>Apply Filters</Button>
            </>
            : (<S.LoadingAnimation src='https://images-na.ssl-images-amazon.com/images/G/01/personalization/ybh/loading-4x-gray._CB485916920_.gif' alt />)
          }
        </S.Left>
