...
export const MenuHeader = ({ userName }) => {
  return (
    <Txt.CustomLink to={userName ? "#" : "/signin"}>
      <HeaderContainer>
        <MenuIcon />
        <Txt.H margin="3px 0 0 0" color="white" weight="700" size="19px">
...
const ListItemArrow = styled.i`
  ...
  background-position: ${(props) =>
    props.direction === "bottom" ? "-184px -342px" : "-137px -288px"};
...
const ListItemContainer = styled.li`
  ...
  justify-content: ${(props) =>
    props.direction === "bottom" ? "flex-start" : "space-between"};
  ...
  &:hover ${ListItemArrow} {
    background-position: ${(props) =>
      props.direction === "bottom" ? "-184px -361px" : "-166px -288px"};
  }
`;
...
export const ListItem = ({ arrow = "true", to = "/", onClick, children }) => {
  return (
    <Link to={to} onClick={onClick}>
      <ListItemContainer>
        <Txt.Span size="14px" lineHeight="16px">
          {children}
        </Txt.Span>
        {arrow ? <ListItemArrow /> : ""}
      </ListItemContainer>
